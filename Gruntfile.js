...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'jachik';
    var email     = grunt.option('email')  || 'jacek@uni-koblenz.de';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new stocks name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/stocks.png')) {
            fs.renameSync(__dirname + '/admin/stocks.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/stocks.html')) {
            fs.renameSync(__dirname + '/widgets/stocks.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/stocks/js/stocks.js')) {
            fs.renameSync(__dirname + '/widgets/stocks/js/stocks.js', __dirname + '/widgets/stocks/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/stocks')) {
            fs.renameSync(__dirname + '/widgets/stocks',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Stocks/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Stocks'
                        },
                        {
                            match: /jachik/g,
                            replacement: author
